...
import { createTenantReview } from '../../../actions/post'



const ReviewTenantModal = (props) => {

  const initialPostData = { ownerId:'', ownerName:'', review:'', rate:'', tenantId:'', tenantName:'' }
  ...
  const handleSubmit = (e)=>{
    ...
    dispatch(createTenantReview(postData))
    props?.onHide()
  }

      const owner_id = user?.result?._id ? user?.result?._id : user?.result?.googleId
          ...
          <div>
              <form onSubmit={handleSubmit} >
                      <div className="form-group">
                          <textarea onChange={(e)=> setPostData({...postData, review: e.target.value, ownerId:owner_id, ownerName: user?.result?.name, tenantId: props?.tenantId, tenantName: props?.tenantName })} placeholder='Review' className="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
                      </div>
...
export default ReviewTenantModal
